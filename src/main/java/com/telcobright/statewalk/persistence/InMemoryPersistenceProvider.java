...
public class InMemoryPersistenceProvider implements PersistenceProvider {
    ...
    public void quarantine(String machineId, String registryName, String reason) {
        Map<String, MachineSnapshot> byReg = store.get(machineId);
        if (byReg == null) return;
        MachineSnapshot s = byReg.get(registryName);
        if (s == null) return;
        dead.computeIfAbsent(machineId, k -> new ConcurrentHashMap<>()).put(registryName, s);
        delete(machineId, registryName);
    }

    /** Quarantined snapshots (rehydration failures), kept for inspection. */
    private final Map<String, Map<String, MachineSnapshot>> dead = new ConcurrentHashMap<>();
    ...
    /** Test helper: total quarantined snapshot count. */
    public int deadSize() {
        int total = 0;
        for (var byReg : dead.values()) total += byReg.size();
        return total;
    }

    /** Test helper: clear the store. */
    public void clear() { store.clear(); dead.clear(); }
}
