...
public interface PersistenceProvider {
    ...
    /**
     * Move a snapshot OUT of the live store into a dead-letter area, keeping
     * the data for a fixed build to recover instead of destroying it. Called
     * by the framework when rehydration of this snapshot failed (unknown saved
     * state after a deploy, undeserializable context, corrupt payload) — the
     * v2 behaviour of deleting the only recovery copy on a rehydrate failure
     * is gone.
     *
     * <p>Must be idempotent; quarantining a missing key is a no-op. A provider
     * that cannot implement this should leave the row in place (the default) —
     * the framework tracks the id in memory to avoid retry loops, and startup
     * recovery skips it with an ERROR log until an operator intervenes.
     */
    default void quarantine(String machineId, String registryName, String reason) {
        // default: leave the row where it is; the registry logs and skips it
    }
}
